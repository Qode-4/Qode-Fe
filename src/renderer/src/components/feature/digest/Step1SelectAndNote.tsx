...
export const Step1SelectAndNote = ({
  ...
  return (
    <div className="flex flex-col gap-4">
      <header className="flex items-center justify-between text-xs text-text-subtle">
        <span>
          공유할 대화 {selectedCount}개 · 최대 {MAX_SHARE_PAIRS}개
        </span>
        {selectedCount === 1 ? (
          <span className="text-text-subtle">마지막 1개는 해제할 수 없습니다.</span>
        ) : null}
      </header>
      ...
      >
            ...
            <li key={pair.messageId}>
              ...
              >
                ...
                <div className="flex min-w-0 flex-1 flex-col gap-1">
                  <p className="line-clamp-2 text-sm font-medium text-text-base">{pair.question}</p>
                  <p className="line-clamp-2 text-xs text-text-subtle">{pair.answerPreview}</p>
                </div>
              </label>
            </li>
          );
        ...
        {pairs.length === 0 ? (
          <li className="p-6 text-center text-sm text-text-subtle">공유 가능한 대화가 없습니다.</li>
        ) : null}
      </ul>
      ...
      <label className="block">
        <div className="mb-1 flex items-center justify-between">
          <span className="text-xs font-medium text-text-soft">
            팀에 남길 한 마디 <span className="text-text-subtle">(선택)</span>
          </span>
          <span
            className={['text-xs', remaining < 0 ? 'text-danger' : 'text-text-subtle'].join(' ')}
          >
        ...
        <textarea
          className={[
            'block w-full resize-none rounded-md border bg-surface p-3 text-sm text-text-base outline-none',
            'min-h-[88px] focus:border-primary',
