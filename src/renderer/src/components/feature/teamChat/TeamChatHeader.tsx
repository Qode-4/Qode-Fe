  ...
  return (
    <div className="flex items-center justify-between gap-3 bg-surface px-6 py-3 max-sm:justify-end max-sm:px-3 max-sm:py-2">
      <div className="flex min-w-0 items-baseline gap-2 max-sm:hidden">
        <h1 className="truncate text-ui-20 font-semibold text-fg-default">{chatName}</h1>
        {participants.length > 0 ? (
          <span className="shrink-0 text-ui-12 text-fg-muted">· {participants.length}명 참여</span>
        ) : null}
      </div>
      ...
      <div className="flex shrink-0 items-center gap-2">
        <div className="flex items-center -space-x-2" aria-hidden="true">
          {preview.map((participant) =>
            participant.avatarUrl ? (
              ...
              <span
                key={participant.userId}
                className="inline-flex size-7 items-center justify-center rounded-full border-2 border-surface bg-primary-soft text-ui-10 font-semibold text-fg-primary"
              >
          ...
          {remaining > 0 ? (
            <span className="inline-flex size-7 items-center justify-center rounded-full border-2 border-surface bg-surface-muted text-ui-10 font-medium text-fg-muted">
              +{remaining}
            </span>
          ) : null}
        </div>
        ...
        <ChatItemMenu
          ...
          triggerClassName="inline-flex h-8 w-8 items-center justify-center rounded-md text-fg-subtle transition-colors hover:bg-surface-muted"
          actions={actions}
        />
      </div>
    </div>
